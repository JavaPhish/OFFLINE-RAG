...
The React frontend is in `./frontend/`. See [Frontend README](../frontend/README.md) for setup.
...
The frontend defaults to `http://localhost:5173` and calls the backend `/query` endpoint. Ensure the backend is running before using the UI.

Notes:
- The backend calls the Ollama CLI (ensure `ollama` is installed and on PATH).
- Reindexing is done automatically if files change; use `/reindex` to force a reindex.
- Add PDFs to `./data/` to be indexed (requires `pypdf`).
